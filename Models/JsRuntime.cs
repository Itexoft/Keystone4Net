...
public class KeystoneJsObject
{
    public KeystoneJsObject(string name) : this(null, name)
    {
    }

    public KeystoneJsObject(KeystoneImport? import, string name)
    {
        this.Import = import;
        this.Name = name;
    }

    internal KeystoneImport? Import { get; }
...
public abstract class KeystoneJsFunctionCall(KeystoneImport import, string name)
{
...
public abstract class KeystoneJsFunctionObjectCall : KeystoneJsFunctionCall
{
    ...
    protected KeystoneJsFunctionObjectCall(KeystoneImport import, string name) : base(import, name)
    {
        this.props = this.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public)
            .Where(p => p.Name is not (nameof(this.Arguments) or nameof(this.Name) or nameof(this.Import))).ToArray();
    }
