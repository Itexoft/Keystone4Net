...
""");
            ...
            var contextType = this.dbContext.GetType();
...
""");
        ...
        private Dictionary<string, string> Build()
        {
            ...
            foreach (var property in contextType.GetProperties())
            {
                ...
                var typeName = property.PropertyType.GetCustomAttribute<TableAttribute>()?.Name ?? property.PropertyType.Name;
                var listAttr = entityType.GetCustomAttribute<KeystoneListAttribute>() ?? new KeystoneListAttribute(typeName);
            }
        ...
        {
            var provider = this.dbContext.Database.ProviderName?.ToLowerInvariant() ?? string.Empty;
